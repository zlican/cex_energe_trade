...
func SendMessageWaitingLB(botToken, chatID, text string) error {
	proxy := "http://127.0.0.1:10809"
	proxyURL, err := url.Parse(proxy)
	if err != nil {
		return fmt.Errorf("解析代理地址失败: %w", err)
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		if err != nil {
			...
			if resp.StatusCode != http.StatusOK {
				...
				AddMessageWaitingLB(SavedMessageLB{
					Text:      text,
					Timestamp: time.Now(),
				})
...
// AddMessage 添加一条消息，超出maxSize自动删除最早的
func AddMessageWaitingLB(msg SavedMessageLB) {
	savedMessagesWaitingLB.Lock()
	defer savedMessagesWaitingLB.Unlock()

	if len(savedMessagesWaitingLB.messages) >= savedMessagesWaitingLB.maxSize {
		// 删除最早的一条，保持长度不变
		savedMessagesWaitingLB.messages = savedMessagesWaitingLB.messages[1:]
	}
	savedMessagesWaitingLB.messages = append(savedMessagesWaitingLB.messages, msg)
}

// GetLatestMessages 返回最新1条，倒序
func GetLatestMessagesWaitingLB(n int) []SavedMessageLB {
	savedMessagesWaitingLB.RLock()
	defer savedMessagesWaitingLB.RUnlock()

	total := len(savedMessagesWaitingLB.messages)
	if total == 0 {
		return nil
	}
	...
	res := make([]SavedMessageLB, n)
	for i := 0; i < n; i++ {
		res[i] = savedMessagesWaitingLB.messages[total-1-i]
	}
	return res
}
